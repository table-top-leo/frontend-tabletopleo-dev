...
import { Plus, ChevronRight, ChevronDown, ChevronUp, ChevronLeft, Search, Filter, Pencil, Trash2, Upload, Send, RotateCcw, Coffee, ArrowRight, X, Check } from "lucide-react";
...
const MenuCategory = () => {
  ...
  const totalPages = Math.max(1, Math.ceil(filteredItems.length / 3));
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const pagedItems = filteredItems.slice((safeCurrentPage - 1) * 3, safeCurrentPage * 3);
  ...
  const handleSaveItem = () => {
    ...
    if (form.newCategory.trim()) {
      ...
      if (exists) targetCatId = exists.id;
      else {
        const newCat = { id: Date.now(), name: form.newCategory.trim(), image: "🍽️", items: [] };
    ...
    if (editingItem) {
      setCategories(prev => prev.map(cat => ({
        ...cat,
        items: cat.items.map(item => item.id === editingItem.id ? { ...item, name: form.name, description: form.description, price: parseFloat(form.price), status: form.status, imagePreview: form.imagePreview } : item)
      })));
    } else {
      ...
      setCategories(prev => prev.map(cat => cat.id === targetCatId ? { ...cat, items: [...cat.items, newItem] } : cat));
      setNextId(n => n + 1);
  ...
  const confirmDelete = () => {
    if (itemToDelete) {
      setCategories(prev => prev.map(cat => ({ ...cat, items: cat.items.filter(i => i.id !== itemToDelete) })));
    }
  ...
  const handleAddCategory = () => {
    ...
    if (!exists) {
      const newCat = { id: Date.now(), name: newCatName.trim(), image: "🍽️", items: [] };
      setCategories(prev => [...prev, newCat]);
    ...
    <div className="mc-root">
        ...
        <div className="mc-modal-overlay" onClick={() => setShowDeleteModal(false)}>
          <div className="mc-modal" onClick={e => e.stopPropagation()}>
            <div className="mc-modal-head">
              <span className="mc-modal-title">Delete Item</span>
              <button className="mc-modal-close" onClick={() => setShowDeleteModal(false)}><X size={20} /></button>
            </div>
            <div className="mc-modal-body">
              <p>Are you sure you want to delete this item? This action cannot be undone.</p>
            </div>
            <div className="mc-modal-foot">
              <button className="mc-btn-ghost" onClick={() => setShowDeleteModal(false)}>Cancel</button>
              <button className="mc-btn-danger" onClick={confirmDelete}>Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mc-page-header">
        <div className="mc-page-title-row">
          ...
          <button className="mc-btn-dark mc-add-btn" onClick={() => setShowAddCategoryInline(true)}>
      ...
      <div className="mc-layout">
        <aside className="mc-sidebar">
          <div className="mc-sidebar-head">
            ...
            <button className="mc-btn-outline-sm" onClick={() => setShowAddCategoryInline(true)}>
          ...
          {showAddCategoryInline && (
            <div className="mc-add-cat-inline">
              ...
              <div className="mc-inline-actions">
                <button className="mc-btn-ghost" onClick={() => { setShowAddCategoryInline(false); setNewCatName(""); }}>Cancel</button>
                <button className="mc-btn-dark" onClick={handleAddCategory}>Create</button>
              </div>
            </div>
          )}
          ...
          <div className="mc-cat-list">
            {categories.length === 0 ? (
              <div className="mc-no-category">
                ...
                <button onClick={() => setShowAddCategoryInline(true)}>
        ...
        <div className="mc-main">
          <div className="mc-form-panel">
            <div className="mc-form-head">
              ...
              <button className="mc-collapse-btn" onClick={() => setFormCollapsed(v => !v)}>
            ...
            {!formCollapsed && (
              <div className="mc-form-body">
                <div className="mc-form-row mc-form-cat-row">
                  <div className="mc-form-group mc-fg-half">
                    ...
                    <div className="mc-select-wrap">
                      <select className={`mc-select ${formErrors.category ? "mc-err" : ""}`} value={form.category} onChange={e => { setForm(f => ({ ...f, category: e.target.value, newCategory: "" })); setFormErrors(er => ({ ...er, category: "" })); }}>
                  ...
                  <div className="mc-form-group mc-fg-half">
                    ...
                    <input className="mc-input" type="text" placeholder="Add New Category" value={form.newCategory} onChange={e => { setForm(f => ({ ...f, newCategory: e.target.value, category: "" })); setFormErrors(er => ({ ...er, category: "" })); }} />
                  </div>
                </div>
                ...
                <div className="mc-form-row">
                  <div className="mc-form-group mc-fg-half">
                    ...
                    <div className={`mc-input-icon-wrap ${formErrors.name ? "mc-err" : ""}`}>
                      ...
                      <input className="mc-input-inner" type="text" placeholder="Enter item name" value={form.name} onChange={e => { setForm(f => ({ ...f, name: e.target.value })); setFormErrors(er => ({ ...er, name: "" })); }} />
                    </div>
                  ...
                  <div className="mc-form-group mc-fg-half">
                    ...
                    <div className="mc-input-icon-wrap">
                      ...
                      <input className="mc-input-inner" type="text" placeholder="Enter item description" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
                    </div>
                  </div>
                </div>
                ...
                <div className="mc-form-row">
                  <div className="mc-form-group mc-fg-half">
                    <label className="mc-label">Item Image</label>
                    <div className="mc-upload-zone" onClick={() => imageRef.current?.click()} role="button" tabIndex={0}>
                      <input ref={imageRef} type="file" accept="image/*" style={{ display: "none" }} onChange={handleImageUpload} />
                      {form.imagePreview ? <img src={form.imagePreview} alt="preview" className="mc-upload-preview" /> : (
                        <>
                  ...
                  <div className="mc-form-group mc-fg-half">
                    ...
                    <div className={`mc-input-icon-wrap ${formErrors.price ? "mc-err" : ""}`}>
                      ...
                      <input className="mc-input-inner" type="number" min="0" step="0.01" placeholder="0.00" value={form.price} onChange={e => { setForm(f => ({ ...f, price: e.target.value })); setFormErrors(er => ({ ...er, price: "" })); }} />
                    </div>
                ...
                <div className="mc-form-actions">
                  <button className="mc-btn-reset" onClick={resetForm}><RotateCcw size={14} /> Reset</button>
                  <button className="mc-btn-dark mc-save-btn" onClick={handleSaveItem}><Send size={14} /> {editingItem ? "Update Item" : "Save Item"}</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="mc-items-panel">
            <div className="mc-items-head">
              <h3 className="mc-items-title">Items in {selectedCategory?.name || "—"} ({totalItems})</h3>
              <div className="mc-items-controls">
                <div className="mc-search-wrap">
                  <Search size={14} className="mc-search-icon" />
                  <input className="mc-search-input" type="text" placeholder="Search items..." value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setCurrentPage(1); }} />
                </div>
                ...
                <button className="mc-icon-btn"><Filter size={16} /></button>
              </div>
            </div>
                  ...
                  {pagedItems.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="mc-empty">
                        {categories.length === 0 ? "Start by adding a category and items from the form above." : "No items found. Add your first item using the form."}
                      </td>
                    </tr>
                  ) : pagedItems.map(item => (
                    <tr key={item.id} className="mc-tr">
                      <td className="mc-td">
                        <div className="mc-item-cell">
                          <div className="mc-item-thumb">
                            {item.imagePreview ? <img src={item.imagePreview} alt={item.name} className="mc-thumb-img" /> : <span className="mc-thumb-emoji">☕</span>}
                          </div>
                      ...
                      <td className="mc-td">
                        <span className={`mc-status-badge ${item.status === "Active" ? "mc-status-active" : "mc-status-inactive"}`}>
                          <span className="mc-status-dot" />{item.status}
                        </span>
                      </td>
                      <td className="mc-td mc-td-actions">
                        <button className="mc-action-edit" onClick={() => handleEdit(item)}><Pencil size={13} /> Edit</button>
                        <button className="mc-action-delete" onClick={() => openDeleteModal(item.id)}><Trash2 size={13} /> Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
