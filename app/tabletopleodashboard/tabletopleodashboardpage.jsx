...
import {
  ...
  AlertTriangle, ShoppingBag, CheckCircle2, Clock, ArrowRight,
} from 'lucide-react';
...
const PAY_LABEL = {
  upi: 'UPI', razorpay: 'Razorpay', stripe: 'Stripe',
  paypal: 'PayPal', pay_at_counter: 'At Counter', cash: 'Cash',
};
const PAY_COLOR = {
  upi: '#7c3aed', razorpay: '#3395ff', stripe: '#635bff',
  paypal: '#003087', pay_at_counter: '#b45309', cash: '#16a34a',
};
...
const MENU_ITEMS = [
  ...
  {
    id: 'admin-setup', label: 'Admin Setup', icon: Settings2,
    children: [
...
const AdminDashboardNew = () => {
  ...
  useEffect(() => {
    const fn = (e) => {
      const { page, highlight } = e.detail || {};
      if (page) {
        setActiveMenu(page);
        if (highlight) setHighlightOrder(highlight);
        setBellOpen(false);
      }
    };
    window.addEventListener('ttl_navigate', fn);
    return () => window.removeEventListener('ttl_navigate', fn);
  }, []);
  ...
  const pollOrders = useCallback(async () => {
    try {
      ...
      if (!token) return;
      const res  = await fetch('http://localhost:6163/api/admin/orders', {
      ...
      const incoming = data.data.filter(o => ['PLACED', 'ACCEPTED'].includes(o.orderStatus));

      setSeenIds(prev => {
        const fresh = incoming.filter(o => !prev.has(o.orderId));
        if (fresh.length > 0) {
          setNewOrders(prevN => {
            const existingIds = new Set(prevN.map(x => x.orderId));
            const toAdd = fresh.filter(x => !existingIds.has(x.orderId));
            return [...toAdd, ...prevN].slice(0, 15);
          });
        }
        const next = new Set(prev);
        incoming.forEach(o => next.add(o.orderId));
        return next;
      });
    } catch {}
  }, []);
  ...
  useEffect(() => {
    pollOrders();
    const id = setInterval(pollOrders, POLL_MS);
    return () => clearInterval(id);
  }, [pollOrders]);

  const unreadCount = newOrders.filter(o => !readIds.has(o.orderId)).length;

  const handleBellOrderClick = (order) => {
    setReadIds(prev => new Set([...prev, order.orderId]));
    setBellOpen(false);
  ...
  const renderContent = () => {
    ...
    if (activeMenu === 'notifications') {
      return (
        <div data-afd-theme={dark ? 'dark' : 'light'}>
          <NotificationTableTopLeo dark={dark} />
        </div>
      );
    }
    const ActivePage = [MenuCategory, BusinessInformation, SettingsPage, HelpDeskPage, PaymentSetup].find(
      (_, i) => ['menu-category', 'business-info', 'settings', 'help-desk', 'payment-setup'][i] === activeMenu
    );
    ...
    if (activeMenu === 'home') return (
      <>
        <h1 className="afd-page-title">
          {greeting}, <span style={{ color: '#635bff', fontWeight: 800 }}>{firstName}</span> 👋
        </h1>
        <div className="afd-chart-card">
          ...
          <div className="afd-balance-row">
            <div className="afd-balance-col">
              <div className="afd-balance-hd"><span className="afd-balance-label">DKK balance</span><button className="afd-balance-link">View</button></div>
              <div className="afd-balance-amount">0.00kr</div>
            </div>
            <div className="afd-balance-col">
              <div className="afd-balance-hd"><span className="afd-balance-label">Payouts</span><button className="afd-balance-link">View</button></div>
        ...
        <div className="afd-main">
          <header className="afd-topbar">
            ...
            <div className="afd-topbar__actions">
              ...
              {/* ── BELL WITH REAL-TIME DROPDOWN ─────────────── */}
              <div style={{ position:'relative' }} ref={bellRef}>
                <button
                  className={`afd-topbar__icon-btn ${unreadCount > 0 ? 'bell-new' : ''}`}
                  title="New Orders"
                ...
                {bellOpen && (
                  <div style={{
                    ...
                    width:280,
                    background: dk ? '#1e2130' : '#ffffff',
                    border: `1px solid ${dk ? 'rgba(255,255,255,0.08)' : '#e5e7eb'}`,
                    borderRadius:12,
                    boxShadow: dk ? '0 16px 48px rgba(0,0,0,0.5)' : '0 16px 48px rgba(0,0,0,0.14)',
                    zIndex:9999,
                    animation:'ddSlide 0.18s cubic-bezier(0.34,1.2,0.64,1)',
                    overflow:'hidden',
                  }}
                  ...
                  >
                    ...
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 13px 10px', borderBottom:`1px solid ${dk ? 'rgba(255,255,255,0.06)' : '#f3f4f6'}` }}>
                      <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                        ...
                        {newOrders.length > 0 && (
                          <span style={{ fontSize:9.5, fontWeight:800, background:'#635bff', color:'#fff', borderRadius:20, padding:'1px 6px' }}>
                            {newOrders.length}
                          </span>
                        )}
                      </div>
                      {newOrders.length > 0 && (
                        <button onClick={clearAllBell} style={{ fontSize:10.5, color:dk?'#6b7280':'#9ca3af', background:'none', border:'none', cursor:'pointer', fontFamily:'inherit' }}>
                          Clear
                        </button>
                      )}
                    </div>
                    ...
                    <div style={{ maxHeight:320, overflowY:'auto' }}>
                      {newOrders.length === 0 ? (
                        <div style={{ padding:'28px 16px', textAlign:'center' }}>
                          ...
                          <div style={{ fontSize:10.5, color:dk?'#6b7280':'#9ca3af', marginTop:3 }}>New orders appear here instantly</div>
                        </div>
                        ...
                        const isPaid   = order.paymentStatus === 'PAID' || order.payAtCounter;
                        return (
                          <div
                            key={order.orderId}
                            onClick={() => handleBellOrderClick(order)}
                            style={{
                              display:'flex', alignItems:'center', justifyContent:'space-between', gap:10,
                              padding:'9px 13px',
                              cursor:'pointer',
                              borderBottom:`1px solid ${dk ? 'rgba(255,255,255,0.04)' : '#f9fafb'}`,
                              background: isUnread ? (dk ? 'rgba(99,91,255,0.07)' : 'rgba(99,91,255,0.03)') : 'transparent',
                          ...
                          >
                            <div style={{ display:'flex', alignItems:'center', gap:9, minWidth:0 }}>
                              ...
                              <div style={{ minWidth:0 }}>
                                <div style={{ fontSize:11, fontWeight:600, color:dk?'#94a3b8':'#6b7280', marginBottom:2 }}>
                                  New Order Received
                                </div>
                                <div style={{ fontSize:13, fontWeight:800, color:dk?'#e2e8f0':'#111827', fontFamily:'monospace', lineHeight:1 }}>
                                  {order.orderNumber || order.orderId?.slice(0,14)}
                                </div>
                              </div>
                            </div>
                            <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:4, flexShrink:0 }}>
                              <span style={{ fontSize:14, fontWeight:800, color:'#635bff' }}>₹{amount}</span>
                              <span style={{ fontSize:10, fontWeight:600, color: isPaid ? '#16a34a' : '#f59e0b' }}>
                                {isPaid ? '✓ Paid' : 'Pending'}
                              </span>
                            </div>
                          </div>
                    ...
                    {newOrders.length > 0 && (
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 13px', borderTop:`1px solid ${dk ? 'rgba(255,255,255,0.06)' : '#f3f4f6'}` }}>
                        <span style={{ fontSize:10.5, color:dk?'#6b7280':'#9ca3af' }}>
                          {newOrders.length} pending
                        </span>
                        <button
                          onClick={() => { setBellOpen(false); setActiveMenu('orders'); }}
                          style={{ fontSize:11, fontWeight:700, color:'#635bff', background:'none', border:'none', cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:3 }}
                        >
              ...
              {/* Settings toggle */}
              <button className="afd-topbar__icon-btn" title="Settings" onClick={() => setActiveMenu(p => p === 'settings' ? 'home' : 'settings')}>
                <Settings size={17}/>
              </button>
              ...
              <div className="afd-dropdown-wrap" ref={userRef}>
                <button className="afd-user-btn" onClick={() => setUserDropOpen(o => !o)}>
                  <div className="afd-user-avatar" style={{ overflow:'hidden', padding:0 }}>
                    {user?.logoUrl
                      ? <img src={user.logoUrl} alt="logo" style={{ width:'100%',height:'100%',objectFit:'cover',display:'block',borderRadius:'inherit' }} onError={e => { e.target.style.display = 'none'; }}/>
                      : initials
                    }
                  </div>
                  <span>{firstName}</span>
                    ...
                    <div style={{ padding:'14px', borderBottom:'1px solid #f3f4f6' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                        <div style={{ width:44,height:44,borderRadius:10,overflow:'hidden',flexShrink:0,border:'2px solid #f3f4f6',background:'linear-gradient(135deg,#635bff,#a855f7)',display:'flex',alignItems:'center',justifyContent:'center' }}>
                          {user?.logoUrl ? (
                            <>
                              <img src={user.logoUrl} alt="Business Logo" style={{ width:'100%',height:'100%',objectFit:'cover',display:'block' }} onError={e => { e.target.style.display='none'; e.target.nextSibling.style.display='flex'; }}/>
                              <span style={{ display:'none',color:'#fff',fontWeight:800,fontSize:14,alignItems:'center',justifyContent:'center',width:'100%',height:'100%' }}>{initials}</span>
                            </>
                          ) : (
